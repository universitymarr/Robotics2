function u_vector = newton_euler_algorithm(num_joints, DH, qd, qdd, m, I, r, rc, g0, prismatic_indices)
    ...
    %
    % qd = a vector containing qdots ex: [qd1; qd2; qd3]
    %
    % qdd = a vector containing qddots ex: [qdd1; qdd2; qdd3]
    %
    % m = a vector containing the masses of the links ex: [m1; m2; m3]
    %
    ...
    for i=1:num_joints
        ...
        % This equation represents the angular velocity of the 𝑖-th link as a
        % function of the previous link's angular velocity and the current joint velocity 𝑞𝑑(𝑖).
        % In the context of the Jacobian, this is equivalent to computing the rotational part of
        % the Jacobian at each joint. The term [0;0;1] corresponds to the unit vector in the direction
        % of rotation for a revolute joint (if it's prismatic, it would be different).
